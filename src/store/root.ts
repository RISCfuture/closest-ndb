  ...
  return {
    ...
    getters: {
      closestNDB(state: RootState): NDB | undefined {
        ...
        return minBy(state.NDBs, (ndb: NDB) => geoDistance(state.location!, NDBCoords(ndb)));
      },

      bearingToClosestNDB(state: RootState, getters):
        number | undefined {
        if (isUndefined(state.location)) return undefined;
        const ndb: NDB = getters.closestNDB;
        if (isUndefined(ndb)) return undefined;

        return geoInitialBearing(state.location!, NDBCoords(ndb));
      },

      distanceToClosestNDB(state: RootState, getters):
        number | undefined {
        if (isUndefined(state.location)) return undefined;
        const ndb: NDB = getters.closestNDB;
        if (isUndefined(ndb)) return undefined;
